...
exports.SignUp = (request, response) => {
  ...
  adminM
    .create({ name: a, email: b, password: c, image: d, mobile: e })
    .then((result) => {
      uploadFile(
    ...
    .catch((err) => {
      ...
      return response
        .status(500)
        .json({ error: err, message: "Cannot SignUp" });
    });
};
...
exports.SignIn = (request, response) => {
  ...
  adminM
    ...
    .catch((err) => {
      ...
      return response
        .status(500)
        .json({ error: err, message: "Please Enter Valid Email and Password" });
    });
};
...
exports.Update = (request, response) => {
  if (request.file) {
    ...
    adminM
      .updateOne(
        { _id: request.body.id },
        { $set: { name: a, email: b, password: c, image: d, mobile: e } }
      )
      .then((result) => {
        uploadFile(
          path.join(__dirname, "../", "public/images/") + request.file.filename
      ...
      .catch((err) => {
        ...
        return response
          .status(500)
          .json({ error: err, message: "Cannot Update" });
      });
  } else {
    let a = request.body.name;
    let b = request.body.email;
    ...
    adminM
      .updateOne(
        { _id: request.body.id },
        { $set: { name: a, email: b, password: c, mobile: e } }
      )
      .then((result) => {
        return response.status(200).json(result);
      })
...
exports.isApproved = (request, response) => {
  doctor
    ...
    .catch((err) => {
      ...
      return response
        .status(500)
        .json({ error: err, message: "Cannot Approved" });
    });
};
